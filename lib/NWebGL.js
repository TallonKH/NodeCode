...
function setupWebGLRectangle(canvas, frag, onLoadFunc, preserve = false) {
	...
	for (const unfn in frag.uniforms) {
		...
		if (unf.type == "sampler2D") {
			// sampler does not need rebaking - load immediately
			if (typeof unf.src == "string") {
				loadTexture(gl, unf.src, unf.texIndex, location,
					function() {
						frag.pendingItems--;
				...
				// src is display node pending bake
				const node = unf.src;
				// recompile display node - when it's finished, load it's output texture
				node.recompileCanvas(function() {
					loadTexture(gl, node.lastColorDataUrl, unf.texIndex, location, function() {
						frag.pendingItems--;
						if (frag.pendingItems == 0) {
							alreadyLoaded = true;
							onLoadFunc(glPackage);
